...
int main(int argc, char **argv)
{
	...
	{
		Com.Concatenation_entrees(AP1,AP2); //concaténation des entrees API
		
		Com.publish(); //publication vers commande

		Com.Decoupe_sorties(&AP1,&AP2); //decoupe sortie recues de commande
		
		cout << "Sorties AP1 = " 	<<AP1.sorties_api << endl;
		cout << "Sorties AP2 = "	<<AP2.sorties_api << endl;

		AP1.publish(); //envoie vers APIs
		AP2.publish();
