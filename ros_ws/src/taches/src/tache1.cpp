...
{
	if((numTable == 1)&&(Deposee==1)/*&&(position==1)&&(bras == 1)&&(pince == 1)*/) ///****** <---- MODIFICATION
	{
...
void Tache1::PieceDeposeeCallback(const std_msgs::Int32::ConstPtr& msg) ///****** <---- MODIFICATION
{
	Deposee=1;
}
...
void Tache1::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	while(finTraitement != -1)
	{
		...
		{
			if(1/*(position==1)&&(bras==0)&&(pince==0)*/) ///****** <---- MODIFICATION
			{
...
void Tache1::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	posteDeposee = noeud.subscribe("/Cmde_P1/ProduitPose",10,&Tache1::PieceDeposeeCallback,this); ///****** <---- MODIFICATION
